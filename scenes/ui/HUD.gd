...
const C_BG      := Color(0.06, 0.03, 0.12, 0.96)
const C_BORDER  := Color(0.35, 0.20, 0.60, 1.0)
...
func _panel_style(panel: Control, bg: Color, border: Color, top_w: int, bot_w: int) -> void:
	var s := StyleBoxFlat.new()
	s.bg_color = bg
	s.border_color = border
	s.set_border_width_all(0)
	s.border_width_top = top_w
	s.border_width_bottom = bot_w
